...
function hideDeletedMetricRows(metrics){
  const visible=(rows,items)=>rows.filter(row=>!items.some(item=>item.deleted===true&&(item.id===row.key||(row.key===row.name&&item.name===row.name))));
  return {...metrics,
