...
function stockForm(stockItem=null) {
  ...
  const initialInput=$("[name=initialStock]",modal.root);if(initialInput&&!stockItem)initialInput.required=true;
  $(".modal-cancel",modal.root).onclick=modal.close;
  $("#stock-form",modal.root).onsubmit=async event=>{event.preventDefault();const button=event.submitter;setBusy(button,true);try{
    ...
    const initialStock=Number(stockItem?.initialStock??data.initialStock),price=Number(data.price),yellowAlertQty=Number(data.yellowAlertQty),redAlertQty=Number(data.redAlertQty);
