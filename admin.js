...
let state = null;
const PAYMENT_LABELS = {credit:"Pago Credito",debit:"Pago debito",alias:"Pago Alias",cash:"Pago eft",unknown:"Sin forma de pago"};
...
function metrics() {
  ...
  active.forEach(sale => {
    ...
    const paymentKey=Object.hasOwn(PAYMENT_LABELS,sale.paymentMethod)?sale.paymentMethod:"unknown";
    const payment=payments.get(paymentKey)||{key:paymentKey,label:PAYMENT_LABELS[paymentKey],total:0,count:0};payment.total+=Number(sale.total||0);payment.count+=1;payments.set(paymentKey,payment);
  });
...
function productForm(product={}) {
  ...
  $("#product-form",modal.root).onsubmit=async event=>{
    ...
    try{
      ...
      await saveProduct(product.id,{name:data.name.trim(),abbreviation,description:data.description.trim(),defaultPrice:Number(data.defaultPrice),active:Boolean(data.active),deleted:false,...recorded,imageUrl:selectedImage?.imageUrl||"",thumbUrl:selectedImage?.thumbUrl||""});
...
function renderExports(){
  ...
  $("#export-sales").onclick=()=>{
    ...
    downloadCsv(`ventas-${currentLocation()?.codePrefix||"ubicacion"}.csv`,filtered.map(sale=>({codigo:sale.saleCode,fecha:dateOnly(sale.createdAt),hora:timeOnly(sale.createdAt),ubicacion:sale.locationName,vendedor:sale.sellerName,forma_pago:sale.paymentMethodLabel||"Sin forma de pago",productos:sale.items?.map(i=>i.name).join(" | "),cantidades:sale.items?.map(i=>i.qty).join(" | "),subtotal:sale.totalBeforeDiscounts??sale.subtotal,descuento:storedDiscountTotal(sale),total:sale.total,estado:sale.status})));
  };
