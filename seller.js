...
function renderNewSale() {
  ...
  const productsHtml=productGroups.length?productGroups.map(group=>{const isOpen=state.openProductCategories.has(group.id);return `<details class="seller-product-category" data-product-category="${escapeHtml(group.id)}" ${isOpen?"open":""}><summary><span>${escapeHtml(group.name)}</span><small>${group.items.length} producto${group.items.length===1?"":"s"}</small></summary><div class="product-carousel" aria-label="${escapeHtml(group.name)}">${group.items.map(product=>`<button class="product-tile" data-product="${product.id}"><span class="key-badge">${escapeHtml(product.buttonKey||"")}</span><img loading="lazy" src="${imageOrPlaceholder(product.thumbUrl,product.abbreviation)}" alt=""><strong>${escapeHtml(product.abbreviation)}</strong><small>${escapeHtml(product.productName||"")}</small><small>Stock ${product.currentStock}</small></button>`).join("")}</div></details>`;}).join(""):`<div class="empty">No hay productos disponibles</div>`;
  ...
  renderCartLines();
  $$('[data-product-category]',content).forEach(category=>category.ontoggle=()=>{const id=category.dataset.productCategory;if(!id)return;if(category.open)state.openProductCategories.add(id);else state.openProductCategories.delete(id);});
