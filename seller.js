...
export async function renderSeller(root, profile, onLogout) {
  ...
  destroySeller();
  state = {root, profile, onLogout, locations:[], discounts:[], stock:[], sales:[], locationId:"", cart:new Map(), lastProductId:null, discount:null, paymentMethod:null, editSale:null, view:"new", unsubStock:null, keyboard:null, keyboardActive:true, saving:false, salesFilter:"today", salesLimit:100};
...
function renderNewSale() {
  ...
  $("#clear-cart",content).onclick=async()=>{if(await confirmDialog("¿Querés vaciar el carrito?")){state.cart.clear();state.discount=null;state.paymentMethod=null;state.lastProductId=null;renderNewSale();}};
...
function renderSales(){
  const content=$("#seller-content",state.root);const sales=filteredSales();const active=sales.filter(sale=>sale.status==="active");const total=active.reduce((sum,sale)=>sum+Number(sale.total||0),0);const items=active.reduce((sum,sale)=>sum+Number(sale.totalItems||0),0);
  content.innerHTML=`<div class="page-head"><h1>Mis ventas</h1><button id="back-new" class="btn btn-primary">Nueva venta</button></div><div class="toolbar"><select id="sales-filter"><option value="today" ${state.salesFilter==="today"?"selected":""}>Hoy</option><option value="location" ${state.salesFilter==="location"?"selected":""}>Ubicación actual</option><option value="all" ${state.salesFilter==="all"?"selected":""}>Todas las propias</option></select></div><section class="card seller-sales-summary"><span>Monto total vendido</span><strong>${money(total)}</strong><small>${active.length} ventas activas · ${items} productos vendidos</small></section><div class="sale-list">${sales.map(sale=>`<button class="sale-card" data-sale="${sale.id}"><div class="row"><strong>${escapeHtml(sale.saleCode)}</strong><span class="badge ${sale.status==="active"?"ok":"danger"}">${sale.status==="active"?"Activa":"Anulada"}</span></div><div class="row"><span>${dateTime(sale.createdAt)} · ${escapeHtml(sale.locationName)}</span><strong>${money(sale.total)}</strong></div><small>${sale.totalItems} productos · ${escapeHtml(sale.paymentMethodLabel||"Sin forma de pago")}</small></button>`).join("")||`<div class="empty">No hay ventas para este filtro</div>`}</div>${state.sales.length>=state.salesLimit?`<button id="load-more-own-sales" class="btn btn-secondary btn-block">Cargar más ventas</button>`:""}`;
  $("#back-new",content).onclick=()=>{state.view="new";renderView();};$("#sales-filter",content).onchange=event=>{state.salesFilter=event.target.value;renderSales();};$$('[data-sale]',content).forEach(button=>button.onclick=()=>saleDetail(state.sales.find(s=>s.id===button.dataset.sale)));$("#load-more-own-sales",content)?.addEventListener("click",async event=>{state.salesLimit+=100;setBusy(event.currentTarget,true,"Cargando…");await refreshSales();renderSales();});
}
